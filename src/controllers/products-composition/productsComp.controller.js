...
const productsCompositionController = {
  createData: async (req, res) => {
    try {
      const { id, comp_name, comp_type, comp_is_active, updated_at } = req.body
      const { data, error } = await supabase.from('tb_prd_composition').insert({
        id,
        comp_name,
        comp_type,
        comp_is_active,
  ...
  updateData: async (req, res) => {
    try {
      const { id } = req.params
      const { comp_name, comp_type, comp_is_active, updated_at } = req.body

      // check if Data is existing
      const { data: existingData, error: existingDataError } = await supabase
        .from('tb_prd_composition')
        .select('*')
        .eq('id', id)
...
module.exports = productsCompositionController
