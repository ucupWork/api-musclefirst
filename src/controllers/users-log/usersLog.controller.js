...
const userLogController = {
  createData: async (req, res) => {
    try {
      const { id, log_device, log_latest_active, session_code } = req.body
      const { data, error } = await supabase.from('tb_users_log').insert({
        id,
        log_device,
        log_latest_active,
        session_code
  ...
  updateData: async (req, res) => {
    try {
      const { id } = req.params
      const { log_device, log_latest_active, session_code } = req.body
