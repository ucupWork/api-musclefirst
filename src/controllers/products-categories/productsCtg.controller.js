...
const productsCategoriesController = {
  createData: async (req, res) => {
    try {
      const { id, ct_name, updated_at, slug } = req.body
      const { data, error } = await supabase.from('tb_prd_categories').insert({
        id,
        ct_name,
        updated_at,
        slug
  ...
  updateData: async (req, res) => {
    try {
      const { id } = req.params
      const { ct_name, updated_at, slug } = req.body

      // check if Data is existing
      const { data: existingData, error: existingDataError } = await supabase
        .from('tb_prd_categories')
        .select('*')
        .eq('id', id)
